...
fn test_llm_only_filtering() {
    ...
    assert!(stdout_all.contains("llama3-chat"), "LLM model should appear in unfiltered results");
    assert!(stdout_all.contains("stable-diffusion-xl-vision"), "Vision model should appear in unfiltered results");
    assert!(stdout_all.contains("clip-large-embedding"), "CLIP model should appear in unfiltered results");
    assert!(stdout_all.contains("whisper-audio-tts"), "Audio model should appear in unfiltered results");
    
    assert!(stdout_filtered.contains("llama3-chat"), "LLM model should appear in filtered results");
    assert!(!stdout_filtered.contains("stable-diffusion-xl-vision"), "Vision model should be filtered out");
    assert!(!stdout_filtered.contains("clip-large-embedding"), "CLIP model should be filtered out");
    assert!(!stdout_filtered.contains("whisper-audio-tts"), "Audio model should be filtered out");
    
    assert!(stdout_filtered.contains("🎯 Filtering to LLM models only..."), "Should show filtering message");
}
...
fn test_moe_cpu_offloading_flags() {
    ...
    cmd.args(&["--cpu-moe", "list"])
        .assert()
        .success();
    
    // Test n-cpu-moe flag
    let mut cmd2 = Command::cargo_bin("shimmy").unwrap();
    cmd2.args(&["--n-cpu-moe", "4", "list"])
        .assert()
        .success();
}
