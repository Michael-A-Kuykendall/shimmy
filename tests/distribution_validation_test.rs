...
mod tests {
    ...
    fn test_cargo_install_mlx_syntax() {
        ...
        assert!(cargo_toml.contains("mlx = []"), "MLX feature should be defined");
        assert!(cargo_toml.contains("llama-cuda"), "CUDA feature should be defined");
        assert!(cargo_toml.contains("huggingface"), "HuggingFace feature should be defined");
    }
    ...
    fn test_platform_specific_features() {
        ...
        assert!(cargo_toml.contains("mlx = []"), "MLX feature for Apple Silicon");
        ...
        assert!(cargo_toml.contains("llama-vulkan"), "Vulkan feature for cross-platform");
        
        // Fallback: HuggingFace should be available
        assert!(cargo_toml.contains("huggingface"), "HuggingFace fallback feature");
    }
    ...
    fn test_release_workflow_includes_mlx() {
        ...
        assert!(workflow.contains("mlx"), "Release workflow should include MLX features");
        assert!(workflow.contains("aarch64-apple-darwin"), "Release should build Apple Silicon binaries");
        assert!(workflow.contains("x86_64-apple-darwin"), "Release should build Intel Mac binaries");
    }
    ...
    fn test_documentation_mentions_mlx() {
        ...
        assert!(readme.contains("--features mlx"), "README should mention MLX installation");
        assert!(readme.contains("Apple Silicon"), "README should mention Apple Silicon");
        assert!(readme.contains("cargo install shimmy --features mlx"), "README should have exact command");
    }
    ...
    fn test_homebrew_formula_exists() {
        ...
        if formula_exists {
            ...
            assert!(formula.contains("darwin"), "Formula should reference Darwin platform");
        }
    }
    ...
    fn test_feature_flags_consistency() {
        ...
        assert!(cargo_toml.contains("default = ["), "Should have default features");
        
        // Platform features should be optional
        assert!(cargo_toml.contains("mlx = []"), "MLX should be optional feature");
        assert!(cargo_toml.contains("llama-cuda"), "CUDA should be optional feature");
        
        // Should have convenience feature sets
        assert!(cargo_toml.contains("gpu = ["), "Should have GPU convenience features");
        assert!(cargo_toml.contains("apple = ["), "Should have Apple convenience features");
    }
    ...
    fn test_issue_114_documentation_fix() {
        ...
        assert!(readme.contains("Platform-Specific Installation"), 
                "Should have platform-specific installation section");
        assert!(readme.contains("Apple Silicon"), "Should mention Apple Silicon");
        assert!(readme.contains("MLX Metal acceleration"), "Should mention MLX acceleration");
        
        // Should have verification instructions
        assert!(readme.contains("shimmy gpu-info"), "Should show how to verify GPU support");
        assert!(readme.contains("MLX Backend"), "Should mention MLX backend verification");
    }
}